...
The Alp\"{o}ge (2026) counterexample to the classical Jacobian conjecture
in $\C^3$ does not apply to our setting, because the three working conditions
of our framework (geometric closure, algebraic conjugation, descriptive completeness)
are not satisfied on $\C^3$.
\end{abstract}
...
The classical Jacobian conjecture \cite{keller} asserts that a polynomial map
$F:\C^n\to\C^n$ with constant non-zero Jacobian determinant is globally invertible.
In July 2026, Alp\"{o}ge \cite{alpoge} constructed a counterexample in $\C^3$:
a degree-$7$ map with $\Det(\Jac)\equiv-2$ but three distinct inputs collapse to
the same output. Tao \cite{tao} analyzed the geometric mechanism: introducing
$t=y+1/x$ yields a cubic $P(T)=cT^3-2T^2+bT-2a$, which degenerates at $c=0$,
allowing the third root to escape to projective infinity.

The Alp\"{o}ge counterexample relies on two properties of $\C^3$:
non-compactness (permitting escape to infinity) and characteristic $0$
(where the Frobenius map $\Frob(x)=x^3$ is invisible as a field automorphism).
The construction builds on a line of work in the Soviet school of
complex analysis: Vitushkin's rational maps on punctured planes exhibiting
local injectivity with global collapse, and Pinchuk's \cite{pinchuk} 1994
counterexample to the strong real Jacobian conjecture on $\R^2$.

This note considers a different setting. We work on a finite set $S$,
and replace the pointwise Jacobian with the {\em function table matrix} $\MF$.
The main result is that $\Det(\MF)\neq 0$ is equivalent to $F$ being bijective.
...
Each column $j$ of $\MF$ is the standard basis vector $e_{F(j)}$, i.e.,
...
automorphism; our base field $\GF(3)$ has characteristic $3$, where
$\Frob(x)=x^3$ is the native Frobenius automorphism.
